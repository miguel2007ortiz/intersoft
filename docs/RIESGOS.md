...
| CI del job e2e fallaba de forma 100% determinista (reproducido en 2 runs + 1 rerun) en `tienda-flujo.spec.ts`: el test del panel de Envios navegaba como `ana@elprogreso.co`, pero `seed_demo.py` la crea con rol **CLIENTE** (no ADMINISTRADOR como decía el comentario del spec); `personalGuard` la redirige correctamente al marketplace, así que el test nunca veía el panel. Hallazgo #15, mismo gate de supervisor — no es bug de RBAC, es el test con el usuario equivocado | El test del panel de Envios ahora inicia sesión como `luis@elprogreso.co` (rol EMPLEADO), separado de la compradora que crea el envío (`commit 8689c4d`). Verificado local: e2e completo 3/3. |
