/**
 * Dialogo de confirmacion — el "estas seguro?" que se ve en pantalla
 *
 * Que hace: dibuja el modal que abre ConfirmacionService y devuelve aceptar o
 * cancelar.
 * Donde se usa: montado UNA sola vez en app.html; ninguna pantalla lo incluye.
 * Por que asi: accesible de verdad: atrapa el foco dentro del dialogo, cierra
 * con Escape, y al cerrarse devuelve el foco al boton que lo abrio. El
 * `confirm()` del navegador no permitia nada de esto y ademas algunos
 * navegadores lo suprimen sin avisar.
 */
    ...
    @if (confirmacion.abierta(); as datos) {
      <div class="capa" (click)="cancelar()">
        ...
        >
          ...
          <div class="acciones">
            <button #botonCancelar type="button" class="btn btn-cancelar" (click)="cancelar()">
  ...
  styles: [
    `
      ...
      .dialogo {
        ...
        max-width: 440px;
        width: 100%;
        animation: subir 0.15s ease-out;
      }
      .titulo {
        margin: 0 0 var(--e3);
        font-size: 18px;
